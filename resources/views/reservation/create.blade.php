...
@section('content')
        ...
        <div class="flex justify-between md:flex-row flex-col ">
            ...
            <div class="md:w-2/3  md:border-r border-gray-800 p-2">
                ...
                <div class=" flex items-end mt-8 ms-4">
                    ...
                    <p>
                        <span
                            class=" text-3xl font-bold text-red-500 ms-3 me-1 border border-red-500 p-2 rounded-md">{{ $car->price_per_day }}
                            DH</span>
                        <span
                ...
                <div class="px-6 md:me-8">
                    <form id="reservation_form" action="{{ route('car.reservationStore', ['car' => $car->id]) }}"
                        ...
                        <div class="mt-12 md:block hidden  ">
                            <button type="submit"
                                class="text-white bg-red-500 p-3 w-full rounded-lg font-bold hover:bg-black shadow-xl hover:shadow-none">Order
                                Now</button>
                        </div>
                    </form>
                </div>
            ...
            <div class="md:w-1/3 flex flex-col justify-start items-center">
                <div class="relative mx-3 mt-3 flex h-[200px] w-3/4   overflow-hidden rounded-xl shadow-lg">
                    ...
                    <span
                        class="absolute w-24 h-8 py-1 top-0 left-0 m-2 rounded-full bg-red-500 px-2 text-center text-sm font-medium text-white">{{ $car->reduce }}
                        %
                        OFF</span>
                </div>
    ...
    @for ($i = 0; $i < $car->stars; $i++)
        <svg aria-hidden="true" class="h-4 w-4 fill-yellow-400" viewBox="0 0 20 20"
    ...
    @endfor
    <span
        class="mr-2 ml-3 rounded bg-yellow-400 px-2.5 py-0.5 text-sm font-semibold text-white">{{ $car->stars }}.0</span>
</div>
                ...
                flatpickrElement._flatpickr.config.onChange.push(function(selectedDates, dateStr, instance) {
                    if (selectedDates.length === 2) {
                        ...
                        if (startDate && endDate && startDate <= endDate) {
                            ...
                            $('#total-price span').text(totalPrice + ' Dh');
                        } else {
                            $('#duration span').text('-- days');
                            $('#total-price span').text('-- Dh');
                        }
                    } else {
                        $('#duration span').text('-- days');
                        $('#total-price span').text('-- Dh');
